...
if($order['status']>0){
	$safeOutTradeNo = htmlspecialchars((string)$order['out_trade_no'], ENT_QUOTES, 'UTF-8');
	sysmsg('该订单('.$safeOutTradeNo.')已完成支付，请勿重复发起支付');
}

// 获取订单支付方式ID、支付插件、支付通道、支付费率
if($order['tid']==2 || $order['tid']==4){ //充值余额与购买用户组
	$userrow = $DB->getRow("SELECT `uid`,`gid`,`money`,`mode`,`channelinfo`,`ordername` FROM `pre_user` WHERE `uid`=:uid LIMIT 1", [':uid'=>$conf['reg_pay_uid']]);
