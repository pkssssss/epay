...
if(!isset($_POST['token'], $_SESSION['paypage_token']) || !is_string($_POST['token']) || !is_string($_SESSION['paypage_token']) || !hash_equals($_SESSION['paypage_token'], $_POST['token']))showerrorjson('CSRF TOKEN ERROR');
...
$userrow = $DB->getRow("SELECT `mode`,`ordername`,`channelinfo`,`money` FROM `pre_user` WHERE `uid`=:uid LIMIT 1", [':uid'=>$uid]);
