...
if(!preg_match('/^[a-zA-Z0-9_-]{6,64}$/', $trade_no))showerror('订单号格式不正确');
...
$userrow=$DB->getRow("select codename,username from pre_user where uid=:uid limit 1", [':uid'=>$row['uid']]);
$codename = !empty($userrow['codename'])?$userrow['codename']:$userrow['username'];
$safeCodename = htmlspecialchars((string)$codename, ENT_QUOTES, 'UTF-8');
$safeTradeNo = htmlspecialchars($trade_no, ENT_QUOTES, 'UTF-8');
$safeMoney = htmlspecialchars((string)$row['money'], ENT_QUOTES, 'UTF-8');
$safeEndTime = htmlspecialchars((string)$row['endtime'], ENT_QUOTES, 'UTF-8');
...
<div class="weui-msg">
    ...
    <div class="weui-msg__text-area">
        <h2 class="weui-msg__title">支付成功</h2>
			<h2 class="weui-msg__title"><span style="font-size:38px;font-weight:700;color:#f40;">￥<?php echo $safeMoney?></span></h2>
		<div class="weui-msg__custom-area">
			<div class="weui-cells">
			  <div class="weui-cell weui-cell_example">
				<span class="weui-cell__bd">收款方</span>
				<span class="weui-cell__ft"><strong><?php echo $safeCodename?></strong></span>
			  </div>
			  <div class="weui-cell weui-cell_example">
				<span class="weui-cell__bd">完成时间</span>
				<span class="weui-cell__ft"><?php echo $safeEndTime?></span>
			  </div>
			  <div class="weui-cell weui-cell_example">
				<span class="weui-cell__bd">订单号</span>
				<span class="weui-cell__ft"><?php echo $safeTradeNo?></span>
			  </div>
			</div>
		</div>
    </div>
    ...
    <div class="weui-msg__extra-area">
	        <div class="weui-footer"><p class="weui-footer__links"></p><p class="weui-footer__text">Copyright © <?php echo date("Y")?> <?php echo $safeSiteName?></p></div>
    </div>
</div>
</div>
</div>
