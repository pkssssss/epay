...
$userrow = $DB->getRow("SELECT `uid`,`gid`,`money`,`mode`,`pay`,`cert`,`status`,`username`,`channelinfo`,`qq`,`codename` FROM `pre_user` WHERE `uid`=:uid LIMIT 1", [':uid'=>$uid]);
...
$safeCodename = htmlspecialchars($codename, ENT_QUOTES, 'UTF-8');
$csrf_token = generateSecureToken(16);
          ...
          <table>
              <tbody>
                  <tr>
                      ...
                      <td  class="selTitle"><?php echo $safeCodename?></td>
                  </tr>
              </tbody>
          </table>
      </div>
    <form name="payForm" action="dopay" method="post">
        <input type="hidden" name="uid" id="uid" value="<?php echo intval($uid)?>">
        <input type="hidden" name="token" id="token" value="<?php echo htmlspecialchars($csrf_token, ENT_QUOTES, 'UTF-8')?>">
        <input type="hidden" name="paytype" id="paytype" value="<?php echo htmlspecialchars((string)$type, ENT_QUOTES, 'UTF-8')?>">
		<input type="hidden" name="direct" id="direct" value="<?php echo htmlspecialchars((string)$direct, ENT_QUOTES, 'UTF-8')?>">
		<input type="hidden" name="payer" id="payer" value="<?php echo htmlspecialchars((string)$openId, ENT_QUOTES, 'UTF-8')?>">
		<input type="hidden" name="trade_no" id="trade_no" value="">
        <?php if($money){?><input type="hidden" name="txAmount" id="txAmount" value="<?php echo htmlspecialchars((string)$money, ENT_QUOTES, 'UTF-8')?>"><?php }?>
