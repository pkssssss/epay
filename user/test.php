...
if(isset($_GET['ok']) && isset($_GET['trade_no'])){
	...
	$row=$DB->getRow("SELECT * FROM pre_order WHERE trade_no=:trade_no AND uid=:uid limit 1", [':trade_no'=>$trade_no, ':uid'=>$conf['test_pay_uid']]);
	...
	$userrow = $DB->getRow("SELECT uid,gid FROM pre_user WHERE uid=:uid limit 1", [':uid'=>$conf['test_pay_uid']]);
	if(!$userrow)sysmsg("测试支付商户不存在");
	$paytype = \lib\Channel::getTypes($userrow['uid'], $userrow['gid']);
		$csrf_token = getCsrfToken();
	$money = 1;
}
$safeSiteName = htmlspecialchars((string)$conf['sitename'], ENT_QUOTES, 'UTF-8');
$safeTradeNo = htmlspecialchars((string)$trade_no, ENT_QUOTES, 'UTF-8');
$safeMoney = htmlspecialchars((string)$money, ENT_QUOTES, 'UTF-8');
$safeCsrfToken = htmlspecialchars((string)$csrf_token, ENT_QUOTES, 'UTF-8');
?>
<!DOCTYPE html>
<html lang="zh-CN">
<body>
<head>
	...
	<meta name="viewport" content="width=device-width, initial-scale=1, maximum-scale=1" />
		<title><?php echo $safeSiteName?> - 测试支付</title>
...
<div class="page-header">
	  <h4><?php echo $safeSiteName?> - 测试支付<a href="/" class="pull-right"><small>返回首页</small></a></h4>
</div>
<div class="panel panel-primary">
<div class="panel-body">

<form name="alipayment">
<input type="hidden" name="csrf_token" value="<?php echo $safeCsrfToken?>">
<div class="form-group"><div class="input-group">
<span class="input-group-addon"><span class="glyphicon glyphicon-barcode"></span></span>
<input class="form-control" placeholder="商户订单号" value="<?php echo $safeTradeNo?>" name="trade_no" type="text" disabled="">
</div></div>
<div class="form-group"><div class="input-group">
...
<input class="form-control" placeholder="付款金额" value="<?php echo $safeMoney?>" name="money" type="text" <?php echo isset($_GET['ok'])?'disabled=""':'required=""'?>>	        
</div></div>
<center>
...
<div class="btn-group" role="group">
	  <button type="button" name="type" value="<?php echo intval($rows['id'])?>" class="btn btn-default" onclick="submitPay(this)"><img src="/assets/icon/<?php echo preg_replace('/[^a-zA-Z0-9_-]/', '', (string)$rows['name'])?>.ico" height="18">&nbsp;<?php echo htmlspecialchars((string)$rows['showname'], ENT_QUOTES, 'UTF-8')?></button>
</div>
<?php }?>
</div>
<?php }?>
...
<?php echo $safeSiteName?> © <?php echo date("Y")?> All Rights Reserved.
</div>
</div>
</div>
</div>
