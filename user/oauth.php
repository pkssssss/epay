...
if(isset($_GET['act']) && $_GET['act']=='login'){
	if(isset($_SESSION['alipay_uid']) && !empty($_SESSION['alipay_uid'])){
		$alipay_uid = $_SESSION['alipay_uid'];
		...
		if($userrow){
			...
			$sds=$DB->exec("UPDATE pre_user SET alipay_uid=:alipay_uid WHERE uid=:uid", [':alipay_uid'=>$alipay_uid, ':uid'=>$uid]);
...
if(isset($_GET['auth_code'])){
	$oauthState = isset($_GET['state']) ? trim($_GET['state']) : '';
	if(empty($_SESSION['oauth_alipay_state']) || $oauthState==='' || !hash_equals($_SESSION['oauth_alipay_state'], $oauthState)){
		sysmsg('state校验失败');
	}
	...
	if(isset($_SESSION['oauth_alipay_redirect_state'])){
		$redirectState = $_SESSION['oauth_alipay_redirect_state'];
		unset($_SESSION['oauth_alipay_redirect_state']);
		$redirect_uri = authcode(str_replace(' ', '+', $redirectState), 'DECODE', SYS_KEY);
	...
	$userrow=$DB->getRow("SELECT * FROM pre_user WHERE alipay_uid=:alipay_uid limit 1", [':alipay_uid'=>$user_id]);
	if($userrow){
		$uid=$userrow['uid'];
		$key=$userrow['key'];
		if($islogin2==1){
		...
		$sds=$DB->exec("UPDATE pre_user SET alipay_uid=:alipay_uid WHERE uid=:uid", [':alipay_uid'=>$user_id, ':uid'=>$uid]);
		@header('Content-Type: text/html; charset=UTF-8');
...
}elseif($islogin2==1 && isset($_POST['unbind'])){
	if(!checkRefererHost() || !checkCsrfToken())sysmsg('CSRF TOKEN ERROR');
	$DB->exec("UPDATE pre_user SET alipay_uid=NULL WHERE uid=:uid", [':uid'=>$uid]);
	@header('Content-Type: text/html; charset=UTF-8');
	...
	if(isset($_GET['state']) && trim($_GET['state'])!==''){
		$_SESSION['oauth_alipay_redirect_state'] = trim($_GET['state']);
	}else{
		unset($_SESSION['oauth_alipay_redirect_state']);
	}
	$oauthState = generateSecureToken(16);
	$_SESSION['oauth_alipay_state'] = $oauthState;
	$oauth->oauth($redirect_uri, $oauthState);
}else{

$code_url = $siteurl.'user/oauth.php?'.buildSignedSidQuery(session_id());
if(isset($_GET['bind'])){
	$code_url .= '&bind=1';
}
?>
...
<script>
function jump(){
	var url = <?php echo json_encode($code_url, JSON_UNESCAPED_SLASHES); ?>;
