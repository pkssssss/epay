...
if(isset($_GET['act']) && $_GET['act']=='qrlogin' && $conf['login_qq']==2){
	if(isset($_SESSION['findpwd_qq']) && $qq=$_SESSION['findpwd_qq']){
		$userrow=$DB->getRow("SELECT * FROM pre_user WHERE qq_uid=:qq_uid LIMIT 1", [':qq_uid'=>$qq]);
		...
		if($userrow){
			...
			$sds=$DB->exec("UPDATE pre_user SET qq_uid=:qq_uid WHERE uid=:uid", [':qq_uid'=>$qq, ':uid'=>$uid]);
...
if(isset($_GET['code']) && $_GET['code'] && ($conf['login_qq']==1 || $conf['login_qq']==3 || $conf['login_wx']==-1 || $conf['login_alipay']==-1)){
	if($conf['login_qq']==1 && !isset($_GET['type'])){
		...
		$typecolumn = 'qq_uid';
			...
			$typeMap = [
				'qq' => ['name'=>'QQ', 'column'=>'qq_uid'],
				'wx' => ['name'=>'微信', 'column'=>'wx_uid'],
				'alipay' => ['name'=>'支付宝', 'column'=>'alipay_uid'],
			];
			if(!isset($typeMap[$type])) exit('{"code":-1,"msg":"invalid type"}');
			$typename = $typeMap[$type]['name'];
			$typecolumn = $typeMap[$type]['column'];
			$state = isset($_GET['state']) ? trim($_GET['state']) : '';
			if(empty($_SESSION['Oauth_state']) || $state==='' || !hash_equals($_SESSION['Oauth_state'], $state)){
				sysmsg('state校验失败');
			}
	...
	if($userrow){
		...
		$sds=$DB->exec("UPDATE pre_user SET `{$typecolumn}`=:openid WHERE uid=:uid", [':openid'=>$openid, ':uid'=>$uid]);
		@header('Content-Type: text/html; charset=UTF-8');
	...
	$DB->exec("UPDATE pre_user SET qq_uid=NULL WHERE uid=:uid", [':uid'=>$uid]);
	@header('Content-Type: text/html; charset=UTF-8');
