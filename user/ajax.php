...
case 'login':
	...
	if(!checkCsrfToken())exit('{"code":-1,"msg":"CSRF TOKEN ERROR"}');
		$loginLimitCount = 10;
		$loginLimitSeconds = 900;
		$loginLimitKey = "user_login_lock_".md5($clientip."|".$user);
		$loginLimitData = @unserialize($CACHE->read($loginLimitKey), ["allowed_classes"=>false]);
		if(!is_array($loginLimitData)) $loginLimitData = ["count"=>0,"time"=>0];
		if($loginLimitData["count"] >= $loginLimitCount && $loginLimitData["time"] > time() - $loginLimitSeconds){
			exit('{"code":-1,"msg":"登录失败次数过多，请15分钟后再试"}');
		}
	...
	if($conf['captcha_open_login']==1){
		...
		if($userrow){
			if($type==0){
				$passwordOk = hash_equals((string)$userrow['key'], (string)$pass);
			}else{
				$passwordOk = verifyUserPassword($pass, $userrow['uid'], $userrow['pwd']);
			}
		}
		if($userrow && $passwordOk) {
			$uid = $userrow['uid'];
			if($type==1 && userPasswordNeedsRehash($userrow['pwd'])){
				$newUserPwd = hashUserPassword($pass);
				$DB->update('user', ['pwd'=>$newUserPwd], ['uid'=>$uid]);
				$userrow['pwd'] = $newUserPwd;
			}
		...
		ob_clean();
			setSecureCookie("user_token", $token, $expiretime, "/user", true);
		$DB->exec("update `pre_user` set `lasttime`=NOW() where `uid`=:uid", [':uid'=>$uid]);
			$CACHE->delete($loginLimitKey);
		...
		unset($_SESSION['csrf_token']);
			...
			$loginLimitData["count"] = intval($loginLimitData["count"]) + 1;
			$loginLimitData["time"] = time();
			$CACHE->save($loginLimitKey, $loginLimitData, $loginLimitSeconds);
...
case 'reg':
	...
	if($conf['reg_pay']==1){
			$reg_pay_uid = intval($conf['reg_pay_uid']);
			$urow = $DB->getRow("SELECT uid,gid FROM pre_user WHERE uid=:uid limit 1", [':uid'=>$reg_pay_uid]);
		if(!$urow)exit('{"code":-1,"msg":"注册收款商户ID不存在"}');
		$return_url = $siteurl.'user/reg.php?regok=1';
		$trade_no=date("YmdHis").rand(11111,99999);
		$domain=getdomain($return_url);
			if(!$DB->exec("INSERT INTO `pre_order` (`trade_no`,`out_trade_no`,`uid`,`tid`,`addtime`,`name`,`money`,`notify_url`,`return_url`,`domain`,`ip`,`status`) VALUES (:trade_no, :out_trade_no, :uid, 1, NOW(), :name, :money, :notify_url, :return_url, :domain, :clientip, 0)", [':trade_no'=>$trade_no, ':out_trade_no'=>$trade_no, ':uid'=>$reg_pay_uid, ':name'=>'商户申请', ':money'=>$conf['reg_pay_price'], ':notify_url'=>$return_url, ':return_url'=>$return_url, ':domain'=>$domain, ':clientip'=>$clientip]))
		...
		$uid=$DB->lastInsertId();
		if($sds){
				$pwd = hashUserPassword($pwd);
				$DB->exec("UPDATE pre_user SET pwd=:pwd WHERE uid=:uid", [':pwd'=>$pwd, ':uid'=>$uid]);
			if(!empty($email)){
				$sub = $conf['sitename'].' - 注册成功通知';
					$msg = '<h2>商户注册成功通知</h2>感谢您注册'.$conf['sitename'].'！<br/>您的登录账号：'.(!empty($email)?$email:$phone).'<br/>您的商户ID：'.$uid.'<br/>您的商户秘钥：'.$key.'<br/>'.$conf['sitename'].'官网：<a href="http://'.$_SERVER['HTTP_HOST'].'/" target="_blank">'.$_SERVER['HTTP_HOST'].'</a><br/>【<a href="'.$siteurl.'user/" target="_blank">商户管理后台</a>】';
			...
			unset($_SESSION['csrf_token']);
				if($paystatus == 2){
					\lib\MsgNotice::send('regaudit', 0, ['uid'=>$uid, 'account'=>!empty($email)?$email:$phone]);
				}
		}else{
...
break;
	case 'qrcode':
		...
		$sidQuery = buildSignedSidQuery(session_id());
		if(!empty($conf['localurl_wxpay']) && !strpos($conf['localurl_wxpay'],$_SERVER['HTTP_HOST'])){
			$qrcode = $conf['localurl_wxpay'].'user/openid.php?'.$sidQuery;
		}else{
			$qrcode = $siteurl.'user/openid.php?'.$sidQuery;
		}
