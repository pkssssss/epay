...
<?php if($conf['wxnotice']==1 || $conf['mailnotice']==1){
	$userrow['msgconfig'] = @unserialize($userrow['msgconfig'], ['allowed_classes'=>false]);
	if(!is_array($userrow['msgconfig'])) $userrow['msgconfig'] = [];
	$userrow['msgconfig'] = array_merge(['order'=>0, 'settle'=>0, 'login'=>0, 'complain'=>0, 'order_money'=>0], $userrow['msgconfig']);
?>
				<div class="line line-dashed b-b line-lg pull-in"></div>
...
if(!$group_settings)$group_settings=$DB->getColumn("SELECT settings FROM pre_group WHERE gid=:gid LIMIT 1", [':gid'=>0]);
				...
				<div class="form-group">
					...
					<div class="col-xs-6">
						<?php if($userrow['qq_uid']){?>
							<a class="btn btn-sm btn-success" disabled title="<?php echo $userrow['qq_uid']?>">已绑定</a>
							<form method="POST" action="./connect.php" style="display:inline-block;margin-left:12px;">
								...
								<button type="submit" class="btn btn-sm btn-danger" onclick="return confirm('解绑后将无法通过QQ一键登录，是否确定解绑？');">解绑</button>
							</form>
						<?php }else{?>
				...
				<div class="form-group">
					...
					<div class="col-xs-6">
						<?php if($userrow['wx_uid']){?>
							<a class="btn btn-sm btn-success" disabled title="<?php echo $userrow['wx_uid']?>">已绑定</a>
							<form method="POST" action="./wxlogin.php" style="display:inline-block;margin-left:12px;">
								<input type="hidden" name="unbind" value="1">
								<input type="hidden" name="csrf_token" value="<?php echo htmlspecialchars(getCsrfToken(), ENT_QUOTES, 'UTF-8'); ?>">
								<button type="submit" class="btn btn-sm btn-danger" onclick="return confirm('解绑后将无法通过微信一键登录，是否确定解绑？');">解绑</button>
							</form>
						<?php }else{?>
				...
				<div class="form-group">
					...
					<div class="col-xs-6">
						<?php if($userrow['alipay_uid']){?>
							<a class="btn btn-sm btn-success" disabled title="<?php echo $userrow['alipay_uid']?>">已绑定</a>
							<form method="POST" action="./oauth.php" style="display:inline-block;margin-left:12px;">
								<input type="hidden" name="unbind" value="1">
								<input type="hidden" name="csrf_token" value="<?php echo htmlspecialchars(getCsrfToken(), ENT_QUOTES, 'UTF-8'); ?>">
								<button type="submit" class="btn btn-sm btn-danger" onclick="return confirm('解绑后将无法通过支付宝一键登录，是否确定解绑？');">解绑</button>
							</form>
						<?php }else{?>
