...
if(isset($_POST['submit'])){
	if(!checkRefererHost() || !checkCsrfToken())showmsg('CSRF TOKEN ERROR',3);
	...
	$pwd = trim($_POST['paypwd']);
		$pwdok = verifyUserPassword($pwd, $userrow['uid'], $userrow['pwd']);
		if(empty($pwd) || !$pwdok)showmsg('登录密码输入错误',3);
	...
	if($userrow['settle']==0)showmsg('您的商户出现异常，无法使用代付功能',3);
		if($conf['transfer_maxlimit']>0){
			$a_count = $DB->getColumn('SELECT count(*) FROM pre_transfer WHERE uid=:uid AND type=:type AND account=:account AND paytime>=:paytime', [':uid'=>$uid, ':type'=>$app, ':account'=>$payee_account, ':paytime'=>date('Y-m-d').' 00:00:00']);
