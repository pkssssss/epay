	...
	<a href="?" class="btn btn-default" title="刷新列表"><i class="fa fa-refresh"></i></a>
      ...
      <div class="table-responsive">
        <table class="table table-striped">
          ...
          <tbody>
<?php
$numrows=$DB->getColumn("SELECT count(*) from pre_psreceiver A WHERE{$sql}", $params);
$pagesize=15;
$pages=ceil($numrows/$pagesize);
$page=isset($_GET['page'])?intval($_GET['page']):1;
$offset=$pagesize*($page - 1);

$rs=$DB->query("SELECT A.*,B.name channelname FROM pre_psreceiver A LEFT JOIN pre_channel B ON A.channel=B.id WHERE{$sql} order by A.id desc limit $offset,$pagesize", $params);
while($res = $rs->fetch())
{
