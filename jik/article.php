...
{
	$id=intval($_GET['id']);
	$row=$DB->getRow("select * from pre_article where id=:id limit 1", [':id'=>$id]);
?>
<div class="panel panel-primary">
...
showmsg('保存错误,请确保必填项都不为空!',3);
	...
	$rows=$DB->getRow("select * from pre_article where title=:title limit 1", [':title'=>$title]);
	if($rows)
		showmsg('文章标题已存在！',3);
...
if($_SERVER['REQUEST_METHOD']!=='POST' || !checkRefererHost() || !checkCsrfToken())showmsg('CSRF TOKEN ERROR',3);
		$id=intval($_GET['id']);
		$rows=$DB->getRow("select * from pre_article where id=:id limit 1", [':id'=>$id]);
if(!$rows)
	showmsg('当前记录不存在！',3);
$title=trim($_POST['title']);
$content=$_POST['content'];
...
if(isset($_GET['kw'])){
	...
	$sql=" title LIKE :kw";
	$params = [':kw'=>'%'.$kw.'%'];
	$numrows=$DB->getColumn("SELECT count(*) from pre_article where{$sql}", $params);
