...
$sql = " 1";
$params = [];
$allowColumns = ['channel', 'uid', 'account', 'name'];
if(isset($_GET['value']) && $_GET['value']!=='') {
	$column = isset($_GET['column']) ? trim($_GET['column']) : '';
	if(in_array($column, $allowColumns, true)){
		$value = trim($_GET['value']);
		$sql .= " AND A.`{$column}`=:search_value";
		$params[':search_value'] = $value;
		$link .= '&column='.urlencode($column).'&value='.urlencode($_GET['value']);
	}
}
?>
...
<form action="./ps_receiver2.php" method="GET" class="form-inline">
  <div class="form-group">
    <label><b>搜索</b></label>
	<select name="column" class="form-control" default="<?php echo htmlspecialchars((string)@$_GET['column'], ENT_QUOTES, 'UTF-8')?>"><option value="channel">通道ID</option><option value="uid">商户ID</option><option value="account">接收方账号</option><option value="name">接收方姓名</option></select>
    <input type="text" class="form-control" name="value" placeholder="输入搜索内容" value="<?php echo htmlspecialchars((string)@$_GET['value'], ENT_QUOTES, 'UTF-8')?>">
	...
	<a href="?" class="btn btn-default" title="刷新列表"><i class="fa fa-refresh"></i></a>
      ...
      <div class="table-responsive">
        <table class="table table-striped">
          ...
          <tbody>
<?php
$numrows=$DB->getColumn("SELECT count(*) from pre_psreceiver2 A WHERE{$sql}", $params);
$pagesize=15;
$pages=ceil($numrows/$pagesize);
$page=isset($_GET['page'])?intval($_GET['page']):1;
$offset=$pagesize*($page - 1);

$rs=$DB->query("SELECT A.*,B.name channelname FROM pre_psreceiver2 A LEFT JOIN pre_channel B ON A.channel=B.id WHERE{$sql} order by A.id desc limit $offset,$pagesize", $params);
while($res = $rs->fetch())
{
