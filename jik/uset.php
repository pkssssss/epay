...
unset($rs);
	$list = $DB->getAll("SELECT A.*,B.type,B.name channelname FROM pre_subchannel A LEFT JOIN pre_channel B ON A.channel=B.id WHERE uid=:uid ORDER BY A.id ASC", [':uid'=>$uid]);
?>
