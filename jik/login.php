...
if(isset($_GET['act']) && $_GET['act']=='login'){
  if(!checkRefererHost() || !checkCsrfToken())exit('{"code":403}');
  ...
  if($verifycode==1 && (!$code || strtolower($code) != $_SESSION['vc_code'])){
    exit(json_encode(['code'=>-1,'msg'=>'验证码错误']));
	  ...
	  if(file_exists($login_limit_file)){
	    $login_limit = @unserialize(file_get_contents($login_limit_file), ['allowed_classes'=>false]);
  ...
  if($username == $conf['admin_user'] && verifyAdminPassword($password, $conf['admin_pwd'])){
    if(!isPasswordHashValue($conf['admin_pwd']) || password_needs_rehash($conf['admin_pwd'], PASSWORD_BCRYPT)){
      $newAdminPwd = hashAdminPassword($password);
      saveSetting('admin_pwd', $newAdminPwd);
      $CACHE->clear();
      $conf['admin_pwd'] = $newAdminPwd;
    }
    $DB->insert('log', ['uid'=>0, 'type'=>'登录后台', 'date'=>'NOW()', 'ip'=>$clientip]);
		...
		setSecureCookie("admin_token", $token, $expiretime, '/jik', true);
	...
	if($_SERVER['REQUEST_METHOD'] !== 'POST' || !checkRefererHost() || !checkCsrfToken())exit();
	setSecureCookie("admin_token", "", time() - 2592000, '/jik', true);
