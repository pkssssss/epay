...
$writeActs = ['create_batch','complete_batch','setSettleStatus','opslist','settle_setresult','settle_save','paypwd_input','paypwd_reset','transfer'];
...
case 'settleList':
	...
	if(isset($_POST['value']) && !empty($_POST['value'])) {
		...
		$where[] = "(`account` LIKE :account OR `username` LIKE :username)";
...
case 'setSettleStatus':
	...
	if($status==4){
		...
		if($status==1){
			...
			$sqlParams = [':status'=>$status, ':endtime'=>$date, ':id'=>$id];
			...
			$sqlParams = [':status'=>$status, ':id'=>$id];
		}
		if($DB->exec($sql, $sqlParams)!==false)
			exit('{"code":200}');
		else
...
break;
case 'opslist':
	$status=is_numeric($_POST['status'])?intval($_POST['status']):exit('{"code":-1,"msg":"状态不正确"}');
