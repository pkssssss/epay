...
$writeActs = ['set','setGonggao','delGonggao','setArticle','article_upload'];
...
case 'getcount':
	...
	if($tongji_cachetime+3600>=time() && $tongji_cache && !isset($_GET['getnew'])){
		...
		if(!is_array($array)) $array = ['usermoney'=>0, 'settlemoney'=>0, 'order_today'=>[]];
		$array = array_merge(['usermoney'=>0, 'settlemoney'=>0, 'order_today'=>[]], $array);
		...
		$today=date("Y-m-d");
			$rs=$DB->query("SELECT type,channel,realmoney,profitmoney from pre_order where status=1 and date>=:today", [':today'=>$today]);
		...
		for($i=1;$i<7;$i++){
			...
			if($order_tongji = $CACHE->read('order_'.$day)){
				$order_data = @unserialize($order_tongji, ['allowed_classes'=>false]);
				if(is_array($order_data)) $result["order"][$day] = $order_data;
			}else{
				break;
			}
		}
...
case 'setGonggao':
	...
	if($DB->exec("UPDATE pre_anounce SET status=:status WHERE id=:id", [':status'=>$status, ':id'=>$id]))exit('{"code":0,"msg":"修改状态成功！"}');
	else exit('{"code":-1,"msg":"修改状态失败['.$DB->error().']"}');
break;
case 'delGonggao':
	$id=intval($_GET['id']);
	if($DB->exec("DELETE FROM pre_anounce WHERE id=:id", [':id'=>$id]))exit('{"code":0,"msg":"删除公告成功！"}');
...
break;
	...
	break;
	case 'article_upload':
		$file_name = isset($_FILES['imgFile']['name']) ? $_FILES['imgFile']['name'] : '';
		$tmp_name = isset($_FILES['imgFile']['tmp_name']) ? $_FILES['imgFile']['tmp_name'] : '';
		if(empty($tmp_name) || !is_uploaded_file($tmp_name)){
			exit('{"error":1,"message":"未检测到有效上传文件。"}');
		}
		//获得文件扩展名
		...
		$allow_mime = ['image/gif', 'image/jpeg', 'image/png', 'image/bmp', 'image/x-ms-bmp', 'image/webp'];
		if($image_info===false || !isset($image_info['mime']) || !in_array($image_info['mime'], $allow_mime, true)){
			exit('{"error":1,"message":"上传文件必须是有效图片。"}');
		}
