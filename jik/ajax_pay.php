...
$writeActs = ['setPayType','delPayType','savePayType','setChannel','delChannel','saveChannel','saveChannelInfo','setRoll','delRoll','saveRoll','saveRollInfo','testpay','delWeixin','saveWeixin','testweixin','setWework','delWework','saveWework','refreshWework','testWework'];
...
case 'channelList':
	...
	if(isset($_POST['plugin']) && !empty($_POST['plugin'])) {
		...
		$where[] = "A.`plugin`=:plugin";
		$params[':plugin'] = $plugin;
	}
	if(isset($_POST['dstatus']) && $_POST['dstatus']>-1) {
		$dstatus = intval($_POST['dstatus']);
		$where[] = "A.`status`=:dstatus";
		$params[':dstatus'] = $dstatus;
	}
	if(isset($_POST['kw']) && !empty($_POST['kw'])) {
		$kw = trim($_POST['kw']);
		$where[] = "(A.`id`=:kw_id OR A.`name` LIKE :kw_name)";
		$params[':kw_id'] = $kw;
		$params[':kw_name'] = '%'.$kw.'%';
	}
	$list = $DB->getAll("SELECT A.*,B.name typename,B.showname typeshowname FROM pre_channel A LEFT JOIN pre_type B ON A.type=B.id WHERE ".implode(' AND ', $where)." ORDER BY A.id DESC", $params);
...
case 'delPayType':
	...
	if(!$row)
		exit('{"code":-1,"msg":"当前支付方式不存在！"}');
	$row=$DB->getRow("select * from pre_channel where type=:type limit 1", [':type'=>$id]);
	if($row)
...
case 'getPlugins':
	...
	if(!$type)
		exit('{"code":-1,"msg":"当前支付方式不存在！"}');
	$list=$DB->getAll("SELECT name,showname FROM pre_plugin WHERE types LIKE :types ORDER BY name ASC", [':types'=>'%'.$type.'%']);
	if($list){
...
case 'delChannel':
	...
	if($DB->exec("DELETE FROM pre_channel WHERE id=:id", [':id'=>$id])){
		$DB->exec("DELETE FROM pre_subchannel WHERE channel=:channel", [':channel'=>$id]);
...
case 'channelInfo':
	...
	if(!$row)
		exit('{"code":-1,"msg":"当前支付通道不存在！"}');
	$typename = $DB->getColumn("SELECT name FROM pre_type WHERE id=:id", [':id'=>$row['type']]);
	//if($row['mode']>0){
...
case 'delRoll':
	...
	if(!$row)
		exit('{"code":-1,"msg":"当前轮询组不存在！"}');
	if($DB->exec("DELETE FROM pre_roll WHERE id=:id", [':id'=>$id]))exit('{"code":0,"msg":"删除轮询组成功！"}');
...
case 'rollInfo':
	...
	if(!$row)
		exit('{"code":-1,"msg":"当前轮询组不存在！"}');
	$list=$DB->getAll("select id,name from pre_channel where type=:type and status=1 ORDER BY id ASC", [':type'=>$row['type']]);
...
case 'saveRollInfo':
	...
	if(empty($info))
		exit('{"code":-1,"msg":"通道配置不能为空！"}');
	if($DB->update('roll', ['info'=>$info], ['id'=>$id])!==false)exit('{"code":0,"msg":"修改轮询组成功！"}');
	else exit('{"code":-1,"msg":"修改轮询组失败['.$DB->error().']"}');
break;
...
case 'getSubChannelMoney': //统计子通道金额
	...
	$channel=intval($_GET['channel']);
	$today=$type==1 ? date("Y-m-d", strtotime("-1 day")) : date("Y-m-d");
	$money=$DB->getColumn("SELECT SUM(realmoney) FROM pre_order WHERE date=:today AND subchannel=:channel AND status>0", [':today'=>$today, ':channel'=>$channel]);
	exit('{"code":0,"msg":"succ","money":"'.round($money,2).'"}');
break;
...
case 'testpay':
	...
	if(!$row)
		exit('{"code":-1,"msg":"当前支付通道不存在！"}');
	if($subchannel > 0){
		if(!$DB->getRow("select * from pre_subchannel where id=:id limit 1", [':id'=>$subchannel])) exit('{"code":-1,"msg":"当前子不存在！"}');
	}
