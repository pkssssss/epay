		...
		<form action="./gonggao.php?my=edit_submit&id=<?php echo $id?>" role="form" class="form-horizontal" method="post">
			...
			<div class="list-group-item">
				<div class="input-group">
					...
					<textarea class="form-control" name="content" rows="5" placeholder="输入公告内容" required><?php echo htmlspecialchars($rows['content']); ?></textarea>
				</div>
			</div>
			<div class="list-group-item form-inline">
				...
				<div class="input-group input-colorpicker colorpicker-element">
					<input type="text" name="color" value="<?php echo htmlspecialchars($rows['color']); ?>" class="form-control" placeholder="文字颜色" maxlength="7"/>
...
elseif($my=='add_submit'){
	if(!checkCsrfToken())showmsg('CSRF TOKEN ERROR',3);
	...
	$sds=$DB->exec("INSERT INTO pre_anounce (content, color, sort, addtime, status) VALUES (:content, :color, :sort, NOW(), 1)", [':content'=>$content, ':color'=>$color, ':sort'=>$sort]);
	if($sds){
