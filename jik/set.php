...
$mods=['site'=>'网站信息','pay'=>'支付相关','settle'=>'结算规则','transfer'=>'企业付款','oauth'=>'快捷登录','notice'=>'消息提醒','certificate'=>'实名认证','template'=>'首页模板','gonggao'=>'公告与排版','mail'=>'邮箱与短信','iptype'=>'IP地址','cron'=>'计划任务','proxy'=>'中转代理','account'=>'修改密码'];
?>
...
<div class="panel-body">
  <form onsubmit="return saveSetting(this)" method="post" class="form-horizontal" role="form">
	...
	<div class="form-group">
	  ...
	  <option value="0">私有CDN（su.pksss.cn）</option>
	  <option value="1">南方科大CDN</option>
	  </select></div>
	</div><br/>
	<div class="form-group">
...
<?php
	...
	if(!empty($newpwd) && !empty($newpwd2)){
		if(!verifyAdminPassword($oldpwd, $conf['admin_pwd']))showmsg('旧密码不正确！',3);
		if($newpwd!=$newpwd2)showmsg('两次输入的密码不一致！',3);
		saveSetting('admin_pwd', hashAdminPassword($newpwd));
	}
	$ad=$CACHE->clear();
...
<div class="panel-footer">
          <span class="glyphicon glyphicon-info-sign"></span> 支付密码用于转账接口以及API退款时使用，安装时随机生成，请务必妥善保存并及时修改
        </div>
</div>
<?php
}elseif($mod=='template'){
...
if($errmsg){
	...
	if(is_array($arr) && isset($arr['time']) && isset($arr['errmsg'])){
