...
class EpayCore
{
	...
	public function queryOrder($trade_no){
		$param = $this->buildApiRequestParam(['act'=>'order', 'trade_no'=>$trade_no]);
	...
	public function refund($refund_no, $trade_no, $money){
		...
		$param = $this->buildApiRequestParam(['trade_no'=>$trade_no, 'money'=>$money, 'out_refund_no'=>$refund_no]);
	...
	private function buildApiRequestParam($param){
		$param['pid'] = $this->pid;
		$param['timestamp'] = strval(time());
		return $this->buildRequestParam($param);
	}
