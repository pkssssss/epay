...
$installAdminPwd = null;
$installAdminPayPwd = null;
...
if($step==3){
    ...
    if(!$dbconfig['user']||!$dbconfig['pwd']||!$dbconfig['dbname']) {
        ...
        if(empty($errorMsg) && !$_GET['jump']){
            ...
            $DB->exec("set sql_mode = ''");
            $DB->exec("set names utf8");
            $installAdminPwd = random(12);
            $installAdminPayPwd = random(8, 1);
            ...
            $sqls[]="REPLACE INTO `".$dbqz."_config` VALUES ('admin_pwd', '".$installAdminPwd."')";
            $sqls[]="REPLACE INTO `".$dbqz."_config` VALUES ('admin_paypwd', '".$installAdminPayPwd."')";
...
?>
                ...
                <div class="panel-body">
                    ...
                    <?php
                    $showAdminPwd = $installAdminPwd ? $installAdminPwd : '请到数据库 pre_config 查看 admin_pwd';
                    $showAdminPayPwd = $installAdminPayPwd ? $installAdminPayPwd : '请到数据库 pre_config 查看 admin_paypwd';
                    ?>
                    <ul class="list-group">
                        <li class="list-group-item">1、系统已成功安装完毕！</li>
                        <li class="list-group-item">2、后台地址：<a href="/jik/" target="_blank">/jik/</a> 账号:admin 密码:<?php echo htmlspecialchars($showAdminPwd); ?></li>
                        <li class="list-group-item">3、支付密码：<?php echo htmlspecialchars($showAdminPayPwd); ?>，请及时修改后台管理员密码与支付密码！</li>
