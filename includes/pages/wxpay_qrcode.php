...
<script>
    ...
    function paySuccessRedirect(backurl){
        safeMsg('支付成功，正在跳转中...', {icon: 16, shade: 0.1, time: 15000});
        if(backurl){
    ...
    function loadmsg() {
        $.ajax({
            ...
            dataType: "json",
            url: "/getshop.php",
            data: {type: "wxpay", trade_no: "<?php echo $order['trade_no']?>", sid: "<?php echo $order['trade_no']?>", sid_sign: "<?php echo makeSidSignature($order['trade_no'])?>"},
            success: function (data) {
                if (data.code == 1) {
					paySuccessRedirect(data.backurl);
                }else{
