...
class Channel {
	...
	static public function getSub($id){
		global $DB;
		$value=$DB->getRow("SELECT A.*,B.info FROM pre_subchannel B INNER JOIN pre_channel A ON B.channel=A.id WHERE B.id=:id", [':id'=>$id]);
		if(!$value) return null;
		$channel = ['id'=>$value['id'], 'mode'=>$value['mode'], 'type'=>$value['type'], 'plugin'=>$value['plugin'], 'apptype'=>$value['apptype'], 'appwxmp'=>$value['appwxmp'], 'appwxa'=>$value['appwxa'], 'costrate'=>$value['costrate'], 'daytop'=>$value['daytop']];
	...
	static public function info($id, $gid = 0){
		...
		if($gid>0)$groupinfo=$DB->getColumn("SELECT info FROM pre_group WHERE gid=:gid LIMIT 1", [':gid'=>$gid]);
		...
		if($groupinfo){
			...
			if($channel==0){ //当前商户关闭该通道
				...
				elseif($channel==-1){ //随机可用通道
					$rows=$DB->getAll("SELECT id,plugin,status,rate,apptype,mode,paymin,paymax FROM pre_channel WHERE type=:typeid AND status=1 AND daystatus=0", [':typeid'=>$typeid]);
				if(count($rows)>0){
					$newrows = [];
					foreach($rows as $row){
				...
				elseif($channel==-2){ //用户自定义子通道
					$rows=$DB->getAll("SELECT A.id,plugin,A.status,rate,apptype,mode,paymin,paymax,B.id subid FROM pre_subchannel B INNER JOIN pre_channel A ON B.channel=A.id WHERE B.uid=:uid AND A.type=:typeid AND A.status=1 AND B.status=1 AND daystatus=0 ORDER BY B.usetime ASC", [':uid'=>$uid, ':typeid'=>$typeid]);
				if(count($rows)>0){
					$newrows = [];
					foreach($rows as $row){
						if($money>0 && !empty($row['paymin']) && $row['paymin']>0 && $money<$row['paymin'])continue;
					...
					if(empty($money_rate))$money_rate = $row['rate'];
						$DB->exec("UPDATE pre_subchannel SET usetime=NOW() WHERE id=:id", [':id'=>$row['subid']]);
				...
				//获取轮询组对应通道
					$row=$DB->getRow("SELECT plugin,status,rate,apptype,mode,paymin,paymax FROM pre_channel WHERE id=:id LIMIT 1", [':id'=>$channel]);
			...
			//未设置用户组
			$row=$DB->getRow("SELECT id,plugin,status,rate,apptype,mode,paymin,paymax FROM pre_channel WHERE type=:typeid AND status=1 AND daystatus=0 ORDER BY rand() LIMIT 1", [':typeid'=>$typeid]);
			if($row){
	...
	static public function getTypes($uid, $gid=0){
		...
		if($groupinfo){
			...
			foreach($info as $id=>$row){
				...
				if($row['channel']==0){
					unset($paytype[$id]);
						...
						$channel=$DB->getRow("SELECT rate,status FROM pre_channel WHERE type=:typeid AND status=1 LIMIT 1", [':typeid'=>$id]);
					if(!$channel){
						unset($paytype[$id]);
						...
						$channel=$DB->getRow("SELECT A.id,A.status,rate FROM pre_subchannel B INNER JOIN pre_channel A ON B.channel=A.id WHERE B.uid=:uid AND A.type=:typeid AND A.status=1 AND B.status=1 LIMIT 1", [':uid'=>$uid, ':typeid'=>$id]);
					if(!$channel){
						unset($paytype[$id]);
					}elseif(empty($row['rate'])){
						$paytype[$id]['rate']=$channel['rate'];
						...
						if($row['type']=='roll'){
							$status=$DB->getColumn("SELECT status FROM pre_roll WHERE id=:id LIMIT 1", [':id'=>$row['channel']]);
						}else{
							$status=$DB->getColumn("SELECT status FROM pre_channel WHERE id=:id LIMIT 1", [':id'=>$row['channel']]);
						}
			...
			foreach($paytype as $id=>$row){
					$status=$DB->getColumn("SELECT status FROM pre_channel WHERE type=:typeid AND status=1 limit 1", [':typeid'=>$id]);
					if(!$status || $status==0)unset($paytype[$id]);
					else{
						$paytype[$id]['rate']=$DB->getColumn("SELECT rate FROM pre_channel WHERE type=:typeid AND status=1 limit 1", [':typeid'=>$id]);
					}
				}
			}
		return $paytype;
	...
	static private function getChannelFromRoll($channel, $money){
		global $DB;
		$row=$DB->getRow("SELECT * FROM pre_roll WHERE id=:id LIMIT 1", [':id'=>$channel]);
		if($row['status']==1){
			...
			$channelids = array_values(array_filter(array_map('intval', $channelids), function($id){
				return $id > 0;
			}));
			if(count($channelids) == 0)return false;
			$placeholders = [];
			$params = [];
			foreach($channelids as $k=>$id){
				$key = ':id'.$k;
				$placeholders[] = $key;
				$params[$key] = $id;
			}
			$rows=$DB->getAll("SELECT id,paymin,paymax FROM pre_channel WHERE id IN (".implode(',', $placeholders).") AND status=1 AND daystatus=0", $params);
			$newids = [];
			...
			if($row['kind']==2){
				...
				$index = ($row['index'] + 1) % count($newinfo);
					$DB->exec("UPDATE pre_roll SET `index`=:idx WHERE id=:id", [':idx'=>$index, ':id'=>$row['id']]);
			}
			return $channel;
		}
		return false;
