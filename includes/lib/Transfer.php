...
class Transfer
{
    ...
    //转账状态刷新
    public static function status($out_biz_no, $uid = 0){
        global $DB;
        $orderWhere = ['biz_no' => $out_biz_no];
        if($uid > 0){
            $orderWhere['uid'] = $uid;
        }
        $order = $DB->find('transfer', '*', $orderWhere);
        ...
        if($result['code'] == 0){
            if($result['status'] == 2){
                if($order['status'] == 0){
                    $resCount = $DB->update('transfer', ['status'=>2, 'result'=>$result['errmsg']], $updateWhere);
                ...
                if($order['status'] == 0){
                    $DB->update('transfer', ['status'=>1], $updateWhere);
                }
                $result['msg'] = '转账成功！';
            }else{
    ...
    //转账凭证查询
    public static function proof($out_biz_no, $uid = 0){
        global $DB;
        $orderWhere = ['biz_no' => $out_biz_no];
        if($uid > 0){
            $orderWhere['uid'] = $uid;
