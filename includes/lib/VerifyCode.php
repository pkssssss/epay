...
class VerifyCode{
    ...
    public static function send_code($scene, $type, $sendto, $uid = 0){
        ...
        if($type == 1){
            ...
            $count=$DB->getColumn("select count(*) from pre_regcode where `to`=:phone and time>:thtime", [':phone'=>$phone, ':thtime'=>$thtime]);
            if($count>=self::SMS_PHONE_DAYLY_TIME){
                return '该手机号码发送次数过多，请更换号码！';
            }
            $count=$DB->getColumn("select count(*) from pre_regcode where ip=:ip and time>:thtime", [':ip'=>$clientip, ':thtime'=>$thtime]);
            ...
            $count=$DB->getColumn("select count(*) from pre_regcode where `to`=:email and time>:thtime", [':email'=>$email, ':thtime'=>$thtime]);
    ...
    public static function verify_code($scene, $type, $sendto, $code, $uid = 0){
        ...
        if (!$row) {
            ...
            $DB->exec("update `pre_regcode` set `errcount`=`errcount`+1 where `id`=:id", [':id'=>$row['id']]);
            return '验证码不正确！';
        }
