...
class Transfer
{
    public static function submit(){
        ...
        if($conf['settle_type']==1){
            ...
            $order_today=$DB->getColumn("SELECT SUM(realmoney) from pre_order where uid=:uid and tid<>2 and status=1 and endtime>=:today", [':uid'=>$pid, ':today'=>$today]);
    ...
    public static function query(){
        ...
        if($order['status'] == 1){
            ...
            $result = \lib\Transfer::status($out_biz_no, $pid);
    ...
    public static function proof(){
        ...
        $result = \lib\Transfer::proof($out_biz_no, $pid);
