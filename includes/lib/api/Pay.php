...
class Pay
{
    ...
    public static function query(){
        ...
        if(!empty($queryArr['trade_no'])){
            ...
            $order=$DB->getRow("SELECT * FROM pre_order WHERE uid=:uid AND trade_no=:trade_no LIMIT 1", [':uid'=>$pid, ':trade_no'=>$trade_no]);
        }elseif(!empty($queryArr['out_trade_no'])){
            $out_trade_no=trim($queryArr['out_trade_no']);
            $order=$DB->getRow("SELECT * FROM pre_order WHERE uid=:uid AND out_trade_no=:out_trade_no LIMIT 1", [':uid'=>$pid, ':out_trade_no'=>$out_trade_no]);
        }else{
