...
class Order
{
    public static function freeze($trade_no){
        ...
        if($row['getmoney']>0){
            ...
            $DB->exec("UPDATE pre_order SET status=3 WHERE trade_no=:trade_no", [':trade_no'=>$trade_no]);
        }
    ...
    public static function unfreeze($trade_no){
        ...
        if($row['getmoney']>0){
            ...
            $DB->exec("UPDATE pre_order SET status=1 WHERE trade_no=:trade_no", [':trade_no'=>$trade_no]);
        }
