...
class Payment {
    ...
    static public function processOrder($isnotify, $order, $api_trade_no, $buyer){
        global $DB,$conf,$siteurl,$CACHE;
        $processedFlag = $CACHE->read('orderproc_'.$order['trade_no']);
        $afterCommit = [];

        if($order['status']==0 || $order['status']==4){
            $result = $DB->transaction(function() use ($DB, &$order, $api_trade_no, $buyer, $isnotify, &$afterCommit){
                if(!$DB->exec("UPDATE `pre_order` SET `status`=1 WHERE `trade_no`=:trade_no", [':trade_no'=>$order['trade_no']])) return false;
                ...
                if($DB->update('order', $data, ['trade_no'=>$order['trade_no']]) === false) return false;
                $order['status'] = 1;
                if(!empty($api_trade_no)) $order['api_trade_no'] = $api_trade_no;
                if(!empty($buyer)) $order['buyer'] = $buyer;
                return processOrder($order, $isnotify, $afterCommit, true);
            });
        }else{
            $result = $DB->transaction(function() use ($DB, &$order, $api_trade_no, $buyer, $isnotify, $processedFlag, &$afterCommit){
                if(empty($order['api_trade_no']) && !empty($api_trade_no)){
                ...
                if($isnotify && $order['status']==1 && empty($processedFlag)){
                    return processOrder($order, $isnotify, $afterCommit, true);
                }
                return true;
            });
        ...
        if($result !== false && !empty($afterCommit)){
    ...
    static public function updateOrderProfits($order, $plugin){
        ...
        if(in_array($plugin, $support_plugins)){
            $psreceiver = $DB->getRow("SELECT * FROM `pre_psreceiver` WHERE `channel`=:channel AND `uid`=:uid AND `status`=1", [':channel'=>$order['channel'], ':uid'=>$order['uid']]);
            if(!$psreceiver) $psreceiver = $DB->getRow("SELECT * FROM `pre_psreceiver` WHERE `channel`=:channel AND `uid` IS NULL AND `status`=1", [':channel'=>$order['channel']]);
            if($psreceiver){
    ...
    static public function updateOrderProfits2($order, $plugin){
        ...
        if(in_array($plugin, $support_plugins)){
            $psreceiver = $DB->getRow("SELECT * FROM `pre_psreceiver2` WHERE `channel`=:channel AND `uid`=:uid AND `status`=1", [':channel'=>$order['channel'], ':uid'=>$order['uid']]);
            if(!$psreceiver) $psreceiver = $DB->getRow("SELECT * FROM `pre_psreceiver2` WHERE `channel`=:channel AND `uid` IS NULL AND `status`=1", [':channel'=>$order['channel']]);
            if($psreceiver){
                if(!$psreceiver['minmoney'] || $order['realmoney']>=$psreceiver['minmoney']){
    ...
    public static function alipaydSettle($trade_no, $api_trade_no, $realmoney, $combine = 0, $ext = null){
        ...
        if($combine == 1){
            ...
            $sub_orders = @unserialize($ext, ['allowed_classes'=>false]);
            if(!is_array($sub_orders)) throw new Exception('子单支付结果数据格式错误');
            $failnum = 0;
            $errmsg = '';
    ...
    public static function getWxkfPayUrl($pay_url){
        ...
        if(!$cookiesid||!preg_match('/^[0-9a-z]{32}$/i', $cookiesid)){
            ...
            if(function_exists('setSecureCookie')){
                \setSecureCookie("mysid", $cookiesid, time() + 2592000, '/');
            }else{
        ...
        if($conf['wework_paykfid'] > 0){
            ...
            $placeholders = [];
            $params = [];
            foreach($usekfids as $k=>$id){
                $key = ':aid'.$k;
                $placeholders[] = $key;
                $params[$key] = intval($id);
            }
            $wxkfaccount = $DB->getRow("SELECT A.* FROM pre_wxkfaccount A LEFT JOIN pre_wework B ON A.wid=B.id WHERE A.id NOT IN (".implode(",", $placeholders).") AND B.status=1 ORDER BY A.usetime ASC LIMIT 1", $params);
        }else{
