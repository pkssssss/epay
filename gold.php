...
$sub_mch_id = isset($_GET['sub_mch_id'])?$_GET['sub_mch_id']:'';
$out_trade_no = isset($_GET['out_trade_no'])?trim($_GET['out_trade_no']):'';
$check_code = isset($_GET['check_code'])?$_GET['check_code']:'';
...
if($out_trade_no){
	if(strlen($out_trade_no) > 64){
		...
		if($order){
			...
			$errmsg = '订单号不存在，out_trade_no='.$out_trade_no;
		}
	}
}else{
	$errmsg = '订单号不能为空';
}
$safeErrmsg = htmlspecialchars((string)$errmsg, ENT_QUOTES, 'UTF-8');
$safeMoney = isset($order['money'])?htmlspecialchars((string)$order['money'], ENT_QUOTES, 'UTF-8'):'';
$jumpUrlJson = isset($jump_url)?json_encode((string)$jump_url, JSON_UNESCAPED_UNICODE|JSON_UNESCAPED_SLASHES):'""';
...
<?php }else{?>
    <div class="main">
        <div class="container">
            ...
            <div class="text">￥<?php echo $safeMoney?></div>
            <div class="message">
...
var postData = JSON.stringify(mchData);
parent.postMessage(postData,'https://payapp.weixin.qq.com');
mchData = {action:'jumpOut', jumpOutUrl:<?php echo $jumpUrlJson?>};
