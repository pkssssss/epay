...
if($_GET['do']=='settle'){
	...
	$settle_money = floatval($conf['settle_money']);
	$rs=$DB->query("SELECT * from pre_user where money>=:settle_money and settle=1 and status=1 and account is not null and username is not null", [':settle_money'=>$settle_money]);
	$i=0;
	$allmoney=0;
	while($row = $rs->fetch())
	{
...
elseif($_GET['do']=='order'){
	...
	$regcode_time = time()-3600*24;
	$wxkf_log_thtime = date("Y-m-d H:i:s", strtotime('-48 hours'));

	$CACHE->clean();
	$DB->exec("delete from pre_order where status=0 and addtime<:thtime", [':thtime'=>$thtime]);
	$DB->exec("delete from pre_regcode where `time`<:regcode_time", [':regcode_time'=>$regcode_time]);
	$DB->exec("delete from pre_blacklist where endtime is not null and endtime<NOW()");
	$DB->exec("delete from pay_wxkflog where addtime<:thtime", [':thtime'=>$wxkf_log_thtime]);
	...
	$rs=$DB->query("SELECT type,channel,realmoney,profitmoney from pre_order where status=1 and date>=:lastday and date<:today", [':lastday'=>$lastday, ':today'=>$today]);
...
elseif($_GET['do']=='notify'){
	...
	for($i=0;$i<$limit;$i++){
		...
		if($notify == 2){
			...
			$intervalSecond = 16 * 60;
		}elseif($notify == 4){
			$intervalSecond = 36 * 60;
		}elseif($notify == 5){
			$intervalSecond = 3600;
		}else{
			$DB->exec("UPDATE pre_order SET notify=-1,notifytime=NULL WHERE trade_no=:trade_no", [':trade_no'=>$srow['trade_no']]);
			continue;
		}
		$notifytime = date("Y-m-d H:i:s", time() + $intervalSecond);
		$DB->exec("UPDATE pre_order SET notify=:notify,notifytime=:notifytime WHERE trade_no=:trade_no", [':notify'=>$notify, ':notifytime'=>$notifytime, ':trade_no'=>$srow['trade_no']]);

		$url=creat_callback($srow);
		if(do_notify($url['notify'])){
			$DB->exec("UPDATE pre_order SET notify=0,notifytime=NULL WHERE trade_no=:trade_no", [':trade_no'=>$srow['trade_no']]);
...
elseif($_GET['do']=='check'){
		if($conf['auto_check_channel'] == 1){
			...
			$check_starttime = date("Y-m-d H:i:s", time() - $second);
			$channels = $DB->getAll("SELECT * FROM pre_channel WHERE status=1 ORDER BY id ASC");
			foreach($channels as $channel){
				$channelid = intval($channel['id']);
				$orders=$DB->getAll("SELECT trade_no,status FROM pre_order WHERE addtime>=:starttime AND channel=:channelid ORDER BY trade_no DESC LIMIT {$failcount}", [':starttime'=>$check_starttime, ':channelid'=>$channelid]);
				...
				if($succount == 0){
					$DB->exec("UPDATE pre_channel SET status=0 WHERE id=:id", [':id'=>$channelid]);
		...
		if($conf['auto_check_sucrate'] == 1){
			...
			$check_starttime = date("Y-m-d H:i:s", time() - $second);
			//统计指定时间内每个商户的总订单数量
			$user_all_stats_rows=$DB->getAll("SELECT uid,count(*) ordernum FROM pre_order WHERE addtime>=:starttime GROUP BY uid", [':starttime'=>$check_starttime]);
			//统计指定时间内每个商户的成功订单数量
			$user_suc_stats_rows=$DB->getAll("SELECT uid,count(*) ordernum FROM pre_order WHERE addtime>=:starttime and status>0 GROUP BY uid", [':starttime'=>$check_starttime]);
			$user_suc_stats = [];
		...
		foreach($user_all_stats_rows as $row){
			...
			$user_rate = round($succ_num * 100 / $total_num, 2);
				if($total_num >= $count && $user_rate < $sucrate){
					...
					if($userrow['pay'] == 1){
						$DB->exec("UPDATE pre_user SET pay=0 WHERE uid=:uid", [':uid'=>$row['uid']]);
...
elseif($_GET['do']=='complain'){
	...
	if($channel['plugin'] == 'alipaysl' && substr($channel['appmchid'],0,1)=='['){
		...
		$orders = $DB->getAll("SELECT DISTINCT uid,subchannel FROM pre_order WHERE channel=:channelid AND date>=:startdate", [':channelid'=>$channelid, ':startdate'=>$startdate]);
		...
		$exist = false;
		if(count($uid)>0){
			$uid = array_values(array_unique(array_map('intval', $uid)));
			$uidPlaceholders = [];
			$uidParams = [];
			foreach($uid as $k=>$uidValue){
				$placeholder = ':uid'.$k;
				$uidPlaceholders[] = $placeholder;
				$uidParams[$placeholder] = $uidValue;
			}
			$users = $DB->getAll("SELECT uid,channelinfo FROM pre_user WHERE uid IN (".implode(',', $uidPlaceholders).")", $uidParams);
...
elseif($_GET['do']=='complain_complete'){
	...
	$limit = 10; //每次处理的投诉数量
	$edittime_before = date("Y-m-d H:i:s", time() - $interval);
	$addtime_after = date("Y-m-d H:i:s", strtotime('-3 day'));
	$complain = $DB->getAll("SELECT * FROM pre_complain WHERE status=1 AND paytype=2 AND edittime<:edittime_before AND addtime>:addtime_after ORDER BY id ASC LIMIT {$limit}", [':edittime_before'=>$edittime_before, ':addtime_after'=>$addtime_after]);
	foreach($complain as $row){
		...
		if($result['code'] == 0){
			$DB->exec("UPDATE pre_complain SET status=1 WHERE id=:id", [':id'=>$row['id']]);
